...
comboWidget::comboWidget(const std::string_view s_label, const std::string_view s_text, const std::span<const std::string> items, QWidget* parent) : QWidget(parent){
	layout = new QHBoxLayout();
	setLayout(layout);
	label = new QLabel(QString::fromStdString(std::string(s_label)));
	combo = new QComboBox();
	for(unsigned int i=0;i<items.size();i++){
		combo->addItem(QString::fromStdString(items[i]));
	}
	combo->setCurrentText(QString::fromStdString(std::string(s_text)));
	layout->addWidget(label);
	layout->addWidget(combo);
...
size_t hashStringSpan(const std::span<const std::string> items){
	size_t itemsHash = 0;
	for(unsigned int i=0;i<items.size();i++){
		std::hash_combine(itemsHash, std::string_view(items[i]));
	}
...
bool internal::combo(const std::string_view label, int* current_item, const std::span<const std::string> items){
	std::string_view defaultSelected;
	if(current_item)
		defaultSelected = items[*current_item];
	else
	...
	if(emplaceCorrectQWidget<comboWidget, combo_type>(label, defaultSelected, items)){
		widgets[widgetCounter].valueHash = std::hash<std::string_view>{}(label);
		widgets[widgetCounter].state = comboState{hashStringSpan(items), current_item ? *current_item : 0};
		widgetCounter++;
		return false;
	}

	auto labelHash = std::hash<std::string_view>{}(label);
	size_t itemsHash = hashStringSpan(items);
	comboState& state = std::any_cast<comboState&>(widgets[widgetCounter].state);
	...
	if(state.itemsHash != itemsHash){
		input->combo->clear();
		for(unsigned int i=0;i<items.size();i++){
			input->combo->addItem(QString::fromStdString(items[i]));
		}
		state.itemsHash = itemsHash;
	}
